...
func TestUPIProvider_ValidateLimit(t *testing.T) {
	t.Run("returns nil when amount is within limit", func(t *testing.T) {
		...
		provider := NewUPIProvider(processor, limit, 0.95, 5.0, mockTransaction, mockIdGenerator)

		err := provider.ValidateLimit(50000.0)
		assert.NoError(t, err)
	})

	t.Run("returns nil when amount equals limit", func(t *testing.T) {
		mockTransaction := new(db_test.MockTransactionRepository)
		mockIdGenerator := new(utils_test.MockIdGenerator)
		...
		err := provider.ValidateLimit(limit)
		assert.NoError(t, err)
	})

	t.Run("returns LIMIT_EXCEEDED when amount exceeds limit", func(t *testing.T) {
		mockTransaction := new(db_test.MockTransactionRepository)
		mockIdGenerator := new(utils_test.MockIdGenerator)
		...
		err := provider.ValidateLimit(150000.0)
		assert.Error(t, err)
		assert.Equal(t, failures.LIMIT_EXCEEDED, err)
	})

	t.Run("returns LIMIT_EXCEEDED for zero limit", func(t *testing.T) {
		mockTransaction := new(db_test.MockTransactionRepository)
		mockIdGenerator := new(utils_test.MockIdGenerator)
		...
		err := provider.ValidateLimit(1.0)
		assert.Error(t, err)
		assert.Equal(t, failures.LIMIT_EXCEEDED, err)
	})
}

func TestUPIProvider_GetTransaction(t *testing.T) {
	...
	t.Run("successfully retrieves transaction", func(t *testing.T) {
		...
		processor := make(chan models.ProcessorMessage, 1)
		provider := NewUPIProvider(processor, 100000.0, 0.95, 5.0, mockTransaction, mockIdGenerator)
		...
		assert.Equal(t, models.PaymentChannelUPI, result.Channel)
		assert.Equal(t, 5.0, result.Fee)
...
func TestUPIProvider_Transfer(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("successfully creates and returns transaction", func(t *testing.T) {
		...
		// Note: The implementation uses its own idGenerator, not the one passed in
		// So we can't mock the transaction ID generation
	...
	t.Run("creates transaction with correct fee from provider", func(t *testing.T) {
		...
		fee := 10.0
		processor := make(chan models.ProcessorMessage, 1)
		provider := NewUPIProvider(processor, 100000.0, 0.95, fee, mockTransaction, mockIdGenerator)
		...
		mockTransaction.On("Create", ctx, mock.MatchedBy(func(tx schema.Transaction) bool {
			return tx.Fee == fee &&
				...
				tx.Channel == models.PaymentChannelUPI &&
