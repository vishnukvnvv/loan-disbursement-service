...
func TestIMPSProvider_ValidateLimit(t *testing.T) {
	t.Run("returns nil when amount is within limit", func(t *testing.T) {
		...
		provider := NewIMPSProvider(processor, limit, 0.98, 10.0, mockTransaction, mockIdGenerator)

		err := provider.ValidateLimit(300000.0)
	...
	t.Run("returns LIMIT_EXCEEDED when amount exceeds limit", func(t *testing.T) {
		...
		err := provider.ValidateLimit(600000.0)
		assert.Error(t, err)
...
func TestIMPSProvider_GetTransaction(t *testing.T) {
	...
	t.Run("successfully retrieves transaction", func(t *testing.T) {
		...
		transactionID := "IMPS-TXN-123456789012"
		...
		assert.Equal(t, 300000.0, result.Amount)
		assert.Equal(t, models.PaymentChannelIMPS, result.Channel)
		assert.Equal(t, 10.0, result.Fee)
	...
	t.Run("returns TRANSACTION_NOT_FOUND when transaction does not exist", func(t *testing.T) {
		...
		transactionID := "IMPS-TXN-NONEXISTENT"
...
func TestIMPSProvider_Transfer(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("creates transaction with correct fee from provider", func(t *testing.T) {
		...
		mockTransaction.On("Create", ctx, mock.MatchedBy(func(tx schema.Transaction) bool {
			return tx.Fee == fee &&
				...
				tx.Channel == models.PaymentChannelIMPS &&
